...
public class PathToLeafFromRoot {
    ...
    public static void main(String[] args) {
        ...
        pathToLeafFromRootInRange(root, "", 0, 100, 2);
    }

    private static void pathToLeafFromRootInRange(Node node, String path, int sum, int high, int low) {
        if (node == null) return;
        // This is used to identify the leaf node
        ...
        pathToLeafFromRootInRange(node.left, path + node.data + ", ", sum + node.data, high, low);
        pathToLeafFromRootInRange(node.right, path + node.data + ", ", sum + node.data, high, low);
    }
